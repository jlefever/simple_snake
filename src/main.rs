...
struct Model {
  snake: LinkedList<Location>, // snake, represented as a queue of locations
...
impl Model {
  fn tick(&mut self) {
    // Bind the current location of the head of the snake to x and y 
    let &[x, y] = self.snake.front().unwrap();
    
    // Push a new head onto snake depending on direction facing
    match self.dir {
